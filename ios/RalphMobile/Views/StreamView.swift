...
struct StreamView: View {
    ...
    private var filterCounts: [EventFilter: Int] {
        var toolCalls = 0
        ...
        for event in viewModel.events {
            switch event.type {
            case "tool.call": toolCalls += 1
            case "hat.activated": hatActivations += 1
            case "event.published": events += 1
            default: break
            }
            if event.status == "error" { errors += 1 }
        }
        return [
            .all: viewModel.events.count,
            .toolCalls: toolCalls,
            .hatActivations: hatActivations,
            .events: events,
            .errors: errors
        ]
    }
...
struct EventRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("\(eventTitle), \(isExpanded ? "collapse" : "expand")")
    ...
    private func formatEventJSON() -> String? {
        var dict: [String: Any] = [
            "type": event.type,
            "timestamp": Self.iso8601Formatter.string(from: event.timestamp)
        ]
