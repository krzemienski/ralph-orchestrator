...
struct VerboseEventStreamView: View {
    ...
    private var streamHeader: some View {
        VStack(spacing: 8) {
            HStack {
                ...
                .accessibilityLabel(isVerboseMode ? "Switch to list view" : "Switch to card view")
    ...
    /// Single-pass filter count aggregation (avoids 6 separate .filter{} calls)
    private var filterCounts: [EventCategoryFilter: Int] {
        var hats = 0, gates = 0, tasks = 0, tools = 0, errors = 0
        for event in events {
