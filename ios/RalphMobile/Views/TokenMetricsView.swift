...
struct TokenMetricsView: View {
    ...
    private let backgroundColor = CyberpunkTheme.bgPrimary
    private let accentColor = CyberpunkTheme.accentCyan
    ...
    private func metricItem(label: String, value: String, isHighlighted: Bool = false) -> some View {
        VStack(spacing: 4) {
            ...
            Text(value)
                ...
                .foregroundColor(isHighlighted ? accentColor : CyberpunkTheme.textPrimary)
        }
    ...
    private func formatCost(_ value: Double) -> String {
        Self.costFormatter.maximumFractionDigits = value < 0.01 ? 4 : 2
        return Self.costFormatter.string(from: NSNumber(value: value)) ?? "$\(value)"
    }
