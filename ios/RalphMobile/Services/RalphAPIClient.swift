...
actor RalphAPIClient {
    ...
    init(baseURL: URL, apiKey: String) {
        ...
        config.urlCache = URLCache(memoryCapacity: 2_000_000, diskCapacity: 10_000_000)
    ...
    /// Uses a dedicated ephemeral session with a 3-second timeout to fail fast.
    /// NOTE: Must NOT use `self.session` because its `waitsForConnectivity = true`
    /// prevents the request from timing out when the server is unreachable.
    func checkHealth() async -> Bool {
        ...
        defer { ephemeralSession.invalidateAndCancel() }
        do {
    ...
    /// Automatically injects X-Api-Key header when an API key is configured.
